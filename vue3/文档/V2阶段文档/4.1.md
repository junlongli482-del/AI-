...
│   │       ├── md_editor/         # ✅ MD编辑器API
│   │       │   └── index.js       # 编辑器会话、AI优化
│   │       ├── ai_review/         # ✅ 新增：AI审核API
│   │       │   └── index.js       # 审核提交、状态查询、详情获取
│   │       └── document_publish/  # ✅ 新增：文档发布API
...
**AI审核 (`api/v2/ai_review/index.js`)**：
```javascript
// 审核管理
submitReview(documentId)             // 提交文档审核
getReviewStatus(documentId)          // 查询审核状态
retryReview(documentId)              // 重新审核文档
getReviewDetail(reviewId)            // 获取审核详情

// 历史统计
getReviewHistory(params)             // 获取审核历史记录
getReviewStats()                     // 获取审核统计信息
getRecentReviews(limit)              // 获取最近审核记录

// 配置管理
getReviewConfig()                    // 获取审核配置信息

// 工具函数
getReviewStatusText(status)          // 获取审核状态显示文本
getReviewStatusType(status)          // 获取审核状态标签类型
canRetryReview(status)               // 检查是否可以重新审核
formatReviewDuration(duration)       // 格式化审核时长
shouldShowReviewDetail(reason)       // 验证是否需要显示详情

// 枚举常量
ReviewStatus                         // 审核状态
ReviewStatusText                     // 状态显示文本
```

#### v2 文档发布接口（✅ 新增）

**文档发布 (`api/v2/document_publish/index.js`)**：
```javascript
// 发布管理
submitPublish(data)                  // 提交发布申请
unpublishDocument(documentId, reason) // 撤回已发布文档
getPublishStatus(documentId)         // 获取发布状态
getPublishDetail(documentId)         // 获取发布详情
updatePublishedDocument(documentId, data) // 更新已发布文档

// 文档展示（公开接口）
getPublishedDocuments(params)        // 获取已发布文档列表
incrementViewCount(documentId)       // 增加文档浏览量

// 个人管理
getMyPublishRecords(params)          // 获取我的发布记录
getPublishStats(globalStats)         // 获取发布统计信息

// 配置信息
getPublishConfig()                   // 获取发布配置信息

// 工具函数
getPublishStatusText(status)         // 获取发布状态显示文本
getPublishStatusType(status)         // 获取发布状态标签类型
canPublish(status)                   // 检查是否可以发布
canUnpublish(status)                 // 检查是否可以撤回发布
canUpdatePublish(status, isModified) // 检查是否可以更新发布
formatViewCount(viewCount)           // 格式化浏览量
generateDefaultPublishReason(title)  // 生成默认发布说明
validatePublishData(data)            // 验证发布数据

// 枚举常量
PublishStatus                        // 发布状态
PublishStatusText                    // 状态显示文本
```
...
**用户体验**：
- ✅ 状态标签和图标显示（📝草稿、⏳审核中、✅已发布、❌审核失败）
- ✅ 审核详情对话框（审核信息、失败原因、置信度）
- ✅ 按钮状态智能切换（根据文档状态显示对应操作）
- ✅ 错误提示和成功反馈
- ✅ 响应式设计适配

**已知限制**：
- ⚠️ 更新发布失败后状态逻辑混乱（前端无法区分首次发布失败和更新发布失败）
- ⚠️ 采用简化方案：审核失败统一调用发布接口，功能可用但体验不完美
...
**状态管理复杂性**：
- 文档发布涉及多种状态组合（草稿、审核中、已发布、审核失败）
- 前端状态判断存在局限性，无法完美区分所有业务场景
- 采用简化方案确保功能可用，避免过度复杂化
...
4. 状态管理复杂时，优先考虑简化方案，确保功能可用
5. 组件设计遵循单一职责原则，通过事件实现解耦

---

**🎯 第五阶段开发重点**：
1. **技术广场展示**：文档列表、搜索排序、详情页面
2. **用户体验**：响应式设计、加载状态、空状态处理
3. **性能优化**：分页加载、图片懒加载、缓存策略
4. **SEO优化**：页面标题、meta信息、结构化数据
